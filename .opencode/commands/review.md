...
Spawn three review subagents in parallel. Aggregate findings by severity into `.claude/reviews/<current-branch>-<date>.md`.

## Input

`$ARGUMENTS` - base branch (required, for example `main` or `origin/develop`). No default - user must specify.
...
2. **Track progress:** call `todowrite` with three entries: `security review`, `refactor review`, `bug hunt`. All start `status: pending`.
3. **Mark all three `in_progress`** before dispatch.
4. **Spawn 3 subagents IN PARALLEL** via the `task` tool:
   - `security-reviewer`
   - `refactor-reviewer`
   - `bug-hunter`
   As each returns, update the matching todo entry -> `completed`.
5. **Each subagent receives this prompt shape**:
   ```
