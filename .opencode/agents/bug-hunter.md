...
**Test coverage gaps**
- New behavior with no test in the diff
- Edge case in new code untested (empty, max, negative, null)
- Test that only exercises the happy path

**Regression risk**
- Change to a function with many callers and no test for the modified behavior
- Removed guard / check without replacement

## Severity guide

- **CRITICAL**: logic that corrupts data, deadlock, guaranteed crash on common input
- **HIGH**: unhandled error on hot path, off-by-one on user-visible boundary, missing test for new behavior on critical code
...
- Do not speculate - if a bug depends on runtime config you cannot see, note it as MEDIUM max with `depends on ...` in the problem.
