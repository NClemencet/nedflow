...
If the plan's test or impl sketch is internally inconsistent (for example command runs no test, expected file path does not exist, test would never fail as written), STOP. Return a report explaining the inconsistency. Do not rewrite the plan - escalate to the orchestrator.
